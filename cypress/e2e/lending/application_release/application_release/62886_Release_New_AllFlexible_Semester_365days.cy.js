...
describe('Release New - Default product settings (all flexible) - Semester (365 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.savings_product('SP_LRA_023').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_023', 'MNAME_CL_LRA_023', 'LNAME_CL_LRA_023').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_023').then(loan_product => { loan_product ? (
        ...
        cy.get('@isFallbackSavingsProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedSavingsProduct({
                    productname: 'SP_LRA_023',
                    productcode: 'SP023'
                }).then(() => {
                    cy.savings_product('SP_LRA_023').then(product => {
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_023',
                    firstName: 'FNAME_CL_LRA_023',
                    middleName: 'MNAME_CL_LRA_023'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_023', 'MNAME_CL_LRA_023', 'LNAME_CL_LRA_023').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_023",
                        productcode: "LP023",
                        description: "description for loan product test case 23",
                        termunit: "3",
                        termdefault: "36",
                        termDaysFixed: true,
                    ...
                    deductions: {
                        serviceCharge: {
                            scdiscounteduse: '1', scdiscountedname: 'Service Charge 23', scbracketoption: '2',
                    ...
                    db.loan_product('LP_LRA_023').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
