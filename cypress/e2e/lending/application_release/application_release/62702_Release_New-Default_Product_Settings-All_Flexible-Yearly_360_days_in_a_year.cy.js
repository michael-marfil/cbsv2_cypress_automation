...
describe('Release New - (Bi-Monthly) All fixed but no deduction (365 days in a year)', () => {
    ...
    before(() => {
        ...
        db.savings_product('SP_LRA_008').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_008', 'MNAME_CL_LRA_008', 'LNAME_CL_LRA_008').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_008').then(loan_product => { loan_product ? (
        ...
        cy.get('@isFallbackSavingsProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedSavingsProduct({
                    productname: 'SP_LRA_008',
                    productcode: 'SP008'
                }).then(() => {
                    db.savings_product('SP_LRA_008').then(product => {
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_008',
                    firstName: 'FNAME_CL_LRA_008',
                    middleName: 'MNAME_CL_LRA_008'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_008', 'MNAME_CL_LRA_008', 'LNAME_CL_LRA_008').then(clients => clients 
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: 'LP_LRA_008',
                        productcode: 'LP008',
                        description: 'description for loan product test case 8',
                        termunit: '3',
                    ...
                    deductions: {
                        serviceCharge: {
                            scdiscounteduse: '1', scdiscountedname: 'Service Charge 8', scbracketoption: '2',
                    ...
                    others: {
                        savings: {
                            savingsholdout: 10, savingsproductid: toString(data.savingsProductId)
                        }
                    }
                }).then(() => {
                    db.loan_product('LP_LRA_008').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
        ...
        cy.get('@clientid').then(clientid => {
            cy.get('@accountname').then(accountname => {
                cy.get('@savingsproductid').then(productid => {
                    const client = Number(clientid.trim?.() || clientid);
                    db.check_savings_account(productid, accountname).then(response => {
                        ...
                        db.savings_product(null, response.productid).then(product => {
