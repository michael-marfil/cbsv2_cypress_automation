...
describe('Release New - Default product setting (all flexible) - Weekly (365 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.client_group().then(clientGroup => { data.clientGroup = clientGroup.cliengroupid });
        db.loan_purpose().then(loanpurpose => { data.loanpurposeid = loanpurpose.loanpurposeid });
        db.savings_product('SP_LRA_019').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_019', 'MNAME_CL_LRA_019', 'LNAME_CL_LRA_019').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_019').then(loan_product => { loan_product ? (
        ...
        cy.get('@isFallbackSavingsProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedSavingsProduct({
                    productname: 'SP_LRA_019',
                    productcode: 'SP019'
                }).then(() => {
                    cy.savings_product('SP_LRA_019').then(product => {
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_019',
                    firstName: 'FNAME_CL_LRA_019',
                    middleName: 'MNAME_CL_LRA_019'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_019', 'MNAME_CL_LRA_019', 'LNAME_CL_LRA_019').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_019",
                        productcode: "LP019",
                        description: "description for loan product test case 19",
                        termunit: "3",
                        termdefault: "36",
                        termDaysFixed: true,
                    ...
                    deductions: {
                        serviceCharge: {
                            scdiscounteduse: '1', scdiscountedname: 'Service Charge 19', scbracketoption: '2',
                            scrateoption: '1', scdiscountedflexibility: true
                        }
                    },
                    others: {
                        savings: {
                    ...
                    db.loan_product('LP_LRA_019').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
