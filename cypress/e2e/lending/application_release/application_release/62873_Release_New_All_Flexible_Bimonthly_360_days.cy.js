...
describe('Release New - Default product settings (all flexible) - Bimonthly (360 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.savings_product('SP_LRA_014').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_014', 'MNAME_CL_LRA_014', 'LNAME_CL_LRA_014').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_014').then(loan_product => { loan_product ? (
        ...
        cy.get('@isFallbackSavingsProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedSavingsProduct({
                    productname: 'SP_LRA_014',
                    productcode: 'SP014'
                }).then(() => {
                    db.savings_product('SP_LRA_014').then(product => {
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_014',
                    firstName: 'FNAME_CL_LRA_014',
                    middleName: 'MNAME_CL_LRA_014'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_014', 'MNAME_CL_LRA_014', 'LNAME_CL_LRA_014').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_014",
                        productcode: "LP014",
                        description: "description for loan product test case 14",
                        termunit: "3",
                        termdefault: "36",
                        termDaysFixed: true,
                    ...
                    db.loan_product('LP_LRA_014').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
