...
describe('Release Additional/Reloan - Default product settings (all fixed) - Lumpsum (360 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.clients('FNAME_CL_LRA_033', 'MNAME_CL_LRA_033', 'LNAME_CL_LRA_033').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_033').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_033',
                    firstName: 'FNAME_CL_LRA_033',
                    middleName: 'MNAME_CL_LRA_033'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_033', 'MNAME_CL_LRA_033', 'LNAME_CL_LRA_033').then(clients => {
                        data.clientId = clients.clientid;
                        data.accountName = clients.accountname;
                    });
                });
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                cy.log('reate Loan Product');
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_033",
                        productcode: "LP033",
                        description: "description for loan product test case 33",
                        termunit: "8",
                    ...
                    db.loan_product('LP_LRA_033').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
            ...
            cy.get('@loanproductid').then(loanproductid => {
                db.client_active_loan(clientid).then(response => {
                    ...
                    if (!hasActiveLoan) {
                        ...
                        cy.log('Skipping: Client has no active loan');
                    }
                });
            });
        });
