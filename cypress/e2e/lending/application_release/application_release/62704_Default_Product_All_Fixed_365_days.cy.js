...
describe('Release New - Default product settings (all fixed) - Daily (365 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.clients('FNAME_CL_LRA_010', 'MNAME_CL_LRA_010', 'LNAME_CL_LRA_010').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_010').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_010',
                    firstName: 'FNAME_CL_LRA_010',
                    middleName: 'MNAME_CL_LRA_010'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_010', 'MNAME_CL_LRA_010', 'LNAME_CL_LRA_010').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_010",
                        productcode: "LP010",
                        description: "description for loan product test case 10",
                        termunit: "1",
                        termdefault: "15",
                        requirecoborrower: true
                    },
                }).then(() => {
                    db.loan_product('LP_LRA_010').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
