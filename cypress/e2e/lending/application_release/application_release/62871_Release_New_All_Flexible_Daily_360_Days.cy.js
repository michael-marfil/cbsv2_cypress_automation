...
describe('Release New - Default product settings (all flexible) - Daily (360 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.savings_product('SP_LRA_012').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_012', 'MNAME_CL_LRA_012', 'LNAME_CL_LRA_012').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_012').then(loan_product => { loan_product ? (
        ...
        cy.get('@isFallbackSavingsProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedSavingsProduct({
                    productname: 'SP_LRA_012',
                    productcode: 'SP012'
                }).then(() => {
                    db.savings_product('SP_LRA_012').then(product => {
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_012',
                    firstName: 'FNAME_CL_LRA_012',
                    middleName: 'MNAME_CL_LRA_012'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_012', 'MNAME_CL_LRA_012', 'LNAME_CL_LRA_012').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_012",
                        productcode: "LP012",
                        description: "description for loan product test case 12",
                        termunit: "3",
                        termdefault: "36",
                        termDaysFixed: true,
                    ...
                    db.loan_product('LP_LRA_012').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
                        ...
                        const loan_application_data = {
                            ...
                            data: {
                                ...
                                other_details: [{
                                    ...
                                    loannPurposeTxt: 'This is a sample purpose text',
