...
describe('Release Application with 1402 [1] [2] userright access', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.clients('FNAME_CL_LRA_004', 'MNAME_CL_LRA_004', 'LNAME_CL_LRA_004').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_004').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback.', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_004',
                    firstName: 'MNAME_CL_LRA_004',
                    middleName: 'FNAME_CL_LRA_004'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_004', 'MNAME_CL_LRA_004', 'LNAME_CL_LRA_004').then(clients => clients
    ...
    it('Should seed loan product when using fallback.', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: 'LP_LRA_004',
                        productcode: 'LP004',
                        description: 'description for loan product test case 4'
                    }
                }).then(() => {
                    db.loan_product('LP_LRA_004').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
