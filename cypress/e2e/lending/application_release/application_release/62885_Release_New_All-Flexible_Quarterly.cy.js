...
describe('Release New - Default product settings (all flexible) - Quarterly (365 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.savings_product('SP_LRA_022').then(products => { products ? (
        ...
        db.clients('FNAME_CL_LRA_022', 'MNAME_CL_LRA_022', 'LNAME_CL_LRA_022').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_022').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_022',
                    firstName: 'FNAME_CL_LRA_022',
                    middleName: 'MNAME_CL_LRA_022'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_022', 'MNAME_CL_LRA_022', 'LNAME_CL_LRA_022').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_022",
                        productcode: "LP022",
                        description: "description for loan product test case 22",
                        termunit: "3",
                        termdefault: "36",
                        termDaysFixed: true,
                    ...
                    db.loan_product('LP_LRA_022').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
