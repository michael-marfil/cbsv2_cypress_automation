...
describe('Release New - Default product settings ( all fixed ) - Daily ( 360 days in a year ) but with no deductions', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.clients('FNAME_CL_LRA_005', 'MNAME_CL_LRA_005', 'LNAME_CL_LRA_005').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_005').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_005',
                    firstName: 'FNAME_CL_LRA_005',
                    middleName: 'MNAME_CL_LRA_005'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_005', 'MNAME_CL_LRA_005', 'LNAME_CL_LRA_005').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: 'LP_LRA_005',
                        productcode: 'LP005',
                        description: 'description for loan product test case 5',
                        termunit: '1',
                        termdefault: '25',
                        termmaximum: '60',
                    ...
                    db.loan_product('LP_LRA_005').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
