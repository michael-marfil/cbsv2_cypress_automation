...
describe('Release Additional/Reloan - Default product settings (all fixed) - Yearly (360 days in a year)', () => {
    let data = {};
    ...
    before(() => {
        ...
        db.clients('FNAME_CL_LRA_032', 'MNAME_CL_LRA_032', 'LNAME_CL_LRA_032').then(clients => { clients ? (
            data.clientId = clients.clientid,
            data.accountName = clients.accountname,
            data.clientFallback = false
        ) : data.clientFallback = true });
        db.loan_product('LP_LRA_032').then(loan_product => { loan_product ? (
    ...
    it('Should seed client when using fallback', () => {
        cy.get('@isFallbackClient').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedClient({
                    lastName: 'LNAME_CL_LRA_032',
                    firstName: 'FNAME_CL_LRA_032',
                    middleName: 'MNAME_CL_LRA_032'
                }).then(() => {
                    db.clients('FNAME_CL_LRA_032', 'MNAME_CL_LRA_032', 'LNAME_CL_LRA_032').then(clients => clients
    ...
    it('Should seed loan product when using fallback', () => {
        cy.get('@isFallbackLoanProduct').then(isFallback => {
            if (isFallback) {
                ...
                cy.seedLoanProduct({
                    general: {
                        productname: "LP_LRA_032",
                        productcode: "LP032",
                        description: "description for loan product test case 32",
                        termunit: "7",
                    ...
                    db.loan_product('LP_LRA_032').then(loan_product => {
                        data.loanProductId = loan_product.loanproductid;
                        data.loanProductName = loan_product.name;
                        data.loanProductCode = loan_product.shortname;
                    });
